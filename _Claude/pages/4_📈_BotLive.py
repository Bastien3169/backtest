...
if is_mainnet:
    ...
    _prix_prise  = _prix_ref          # mid HL du symbole, déjà lu en section 2

    # Le panneau de saisie vit DANS le bouton (st.popover) : rien de plus sur la
    # page tant qu'on ne clique pas, et une fois ouvert on retrouve exactement
    # les mêmes briques que l'onglet ➕ Renforcer — montant, TP/SL en % ou en
    # prix, récapitulatif en dollars — au lieu d'aller régler tout ça en
    # section 2 puis redescendre à l'aveugle.
    with st.popover("🎯 Prendre une position sur " + str(_sym_prise),
                    disabled=_hl_indispo):

        st.markdown("**Ouverture manuelle — " + str(_sym_prise) + "**")
        st.caption(
            "Pré-rempli avec la section 2 à la première ouverture, puis le panneau "
            "garde tes dernières valeurs. Ça n'agit que sur cette entrée."
        )

        # Sens choisi ICI : le bouton servait auparavant uniquement le sens du
        # bot (section 2), ce qui obligeait à reconfigurer la stratégie pour
        # ouvrir une position dans l'autre sens.
        _short_prise = st.radio(
            "Sens de la position",
            ["🟢 LONG", "🔴 SHORT"],
            horizontal=True,
            index=1 if is_short else 0,
            key="prise_sens",
        ).endswith("SHORT")
        _sens_prise = "SHORT" if _short_prise else "LONG"

        if _short_prise != is_short:
            st.warning(
                "⚠️ Sens opposé à la section 2 (configurée en "
                + ("SHORT" if is_short else "LONG") + "). Les indicateurs "
                "d'entrée et de sortie du bot sont écrits pour l'autre sens : "
                "si tu démarres le bot après cette ouverture, vérifie d'abord "
                "la section 2."
            )

        _mode_montant = st.radio(
            "Montant exprimé en",
            ["USDC", "% du solde"],
            horizontal=True,
            key="prise_mode_montant",
            help="USDC = la marge que tu engages. % = part de ton solde HL, "
                 "comme le fait le bot automatique.",
        )

        _marge_defaut = round(_solde_prise * (size_pct / 100), 2)
        if _mode_montant == "% du solde":
            _pct_prise   = st.number_input(
                "% du solde HL", 1.0, 100.0, float(size_pct), 1.0, key="prise_pct",
            )
            _marge_prise = _solde_prise * (_pct_prise / 100.0)
        else:
            _marge_prise = st.number_input(
                "Marge engagée (USDC)", 0.0, None, float(_marge_defaut), 10.0,
                key="prise_usd",
                help="Solde HL disponible : " + format(_solde_prise, ".2f") + " USDC.",
            )
        _notional_prise = float(_marge_prise) * leverage

        # Même widget que le renfort : % ou prix de déclenchement, avec le gain
        # et la perte en dollars affichés sous les champs.
        _tpsl_prise = saisie_tp_sl(
            "prise", float(_notional_prise), _prix_prise, _short_prise,
            tp_defaut=float(tp_pct) if tp_pct else 0.0,
            sl_defaut=float(sl_pct) if sl_pct else 0.0,
            libelle_tp="TP de la position",
            libelle_sl="SL de la position",
        )
        _tp_pct_prise = _tpsl_prise["tp_pct"] if _tpsl_prise["tp_pct"] > 0 else None
        _sl_pct_prise = _tpsl_prise["sl_pct"] if _tpsl_prise["sl_pct"] > 0 else None

        st.caption(
            "➡️ Entrée au marché sur **" + str(_sym_prise) + "** en **" + _sens_prise
            + "** — " + format(float(_marge_prise), ".2f") + " USDC de marge × "
            + str(leverage) + " = **" + format(_notional_prise, ".2f")
            + " USDC de position** · "
            + (("TP " + format(_tp_pct_prise, ".2f") + " %") if _tp_pct_prise else "aucun TP")
            + " / "
            + (("SL " + format(_sl_pct_prise, ".2f") + " %") if _sl_pct_prise else "aucun SL")
        )

        if not _sl_pct_prise:
            st.warning(
                "⚠️ Aucun Stop Loss — la position serait ouverte SANS protection "
                "automatique sur Hyperliquid."
            )
        if not _tpsl_prise["ok"]:
            st.error("Corrige le niveau de TP ou de SL ci-dessus avant d'ouvrir.")

        # Le minimum de ~10 USDC est vérifié ICI et pas seulement après le clic :
        # un bouton cliquable qui échoue systématiquement n'aide personne.
        _marge_ok = float(_marge_prise) >= 10
        if not _marge_ok:
            st.error(
                "Marge trop faible : " + format(float(_marge_prise), ".2f")
                + " USDC (minimum ~10 sur Hyperliquid). Solde disponible : "
                + format(_solde_prise, ".2f") + " USDC."
            )

        _go_prise = st.button(
            "✅ Confirmer l'ouverture " + _sens_prise,
            type="primary", key="btn_prise_position",
            disabled=_hl_indispo or not _tpsl_prise["ok"] or not _marge_ok,
        )
    ...
    if _go_prise:
        try:
            ...
            if _deja is not None:
                ...
                _res_p = (_cli_p.short(_sym_prise, _notional_prise) if _short_prise
                ...
                if not (_res_p and _res_p.get("ok")):
                    ...
                    _s_p["strategy"] = {
                        ...
                        "hl_name":        hl_name,
                        "timeframe":      timeframe,
                        # size_pct reste celui de la section 2 : il pilote les
                        # ENTRÉES FUTURES du bot, pas la position qu'on vient
                        # d'ouvrir à la main.
                        "size_pct":       size_pct,
                        # TP/SL en revanche = ceux réellement posés sur HL à
                        # l'instant. Sinon le bot gérerait cette position avec
                        # des niveaux différents de ceux qui existent vraiment.
                        "tp_pct":         _tp_pct_prise,
                        "sl_pct":         _sl_pct_prise,
                        # Sens réellement ouvert, pas celui de la section 2 :
                        # le bot doit gérer la position qui existe.
                        "is_short":       _short_prise,
                        "check_time_utc": check_time_utc,
                        "interval_min":   interval_min,
                        "ind_entry":      ind_entry,
                        "ind_exit":       ind_exit,
                    ...
                    # UTC : le bot compare cette date à datetime.now(timezone.utc).
                    # En heure locale (UTC+2), une prise après 22 h datait du
                    # lendemain et bloquait l'entrée du bot le jour suivant.
                    _s_p["last_entry_date"] = datetime.now(timezone.utc).strftime("%Y-%m-%d")
                    save_state(_s_p)
                    ...
                    if _tpsl_prise["tp_px"]:
                        _tp_p = _tpsl_prise["tp_px"]
                    elif _tp_pct_prise:
                        _tp_p = (_fill_p * (1 - _tp_pct_prise / 100) if _short_prise
                                 else _fill_p * (1 + _tp_pct_prise / 100))
                    if _tpsl_prise["sl_px"]:
                        _sl_p = _tpsl_prise["sl_px"]
                    elif _sl_pct_prise:
                        _sl_p = (_fill_p * (1 + _sl_pct_prise / 100) if _short_prise
                                 else _fill_p * (1 - _sl_pct_prise / 100))

                    if _tp_p or _sl_p:
                        _r_p   = _cli_p.set_tp_sl(
                            asset=_sym_prise, size=_qty_p, is_short=_short_prise,
    ...
    for _p in _positions_live:
        ...
        with st.container(border=True):
            ...
            if not _ordres_ok:
                ...
                if _ord_sym:
                    # Échelle de prix, du plus haut au plus bas : on retrouve la
                    # lecture d'un graphique. Pour un LONG les TP sont au-dessus
                    # et les SL en dessous, pour un SHORT c'est l'inverse.
                    _tries = sorted(_ord_sym, key=lambda x: (x["trigger_px"] or 0),
                                    reverse=True)
                    # Ordre réellement atteint en premier, dans chaque sens.
                    # LONG  : le prix monte vers le TP le plus BAS, descend vers
                    #         le SL le plus HAUT.
                    # SHORT : exactement l'inverse.
                    def _premier(kind):
                        px = [o["trigger_px"] for o in _ord_sym
                              if o["kind"] == kind and o["trigger_px"]]
                        if not px:
                            return None
                        if kind == "SL":
                            return min(px) if _p["is_short"] else max(px)
                        return max(px) if _p["is_short"] else min(px)

                    _sl_actif = _premier("SL")
                    _tp_actif = _premier("TP")
                    _out = []
                    for _o in _tries:
                        _l = "- " + _ligne_ordre(_o, _p["qty"])
                        if _o["kind"] == "SL" and _o["trigger_px"] == _sl_actif:
                            _l += "  ← **ton stop effectif**"
                        elif _o["kind"] == "TP" and _o["trigger_px"] == _tp_actif:
                            _l += "  ← **atteint en premier**"
                        _out.append(_l)
                    st.markdown("\n".join(_out))
                    if _pct_sl > 101.0:
                        st.caption(
