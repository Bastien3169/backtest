...
Lancement multi-bot : python bot_mainnet.py --config bot_state_long.json
"""
...
# ── Config --config ────────────────────────────────────────────────────────────
# Permet de lancer plusieurs bots en parallèle avec des fichiers JSON différents
# Exemple : python bot_mainnet.py --config bot_state_long.json
...
# Préfixe de log — ex: "bot_state_long.json" → "[LONG]"
config_name = args.config.replace("bot_state_", "").replace(".json", "").upper()
BOT_PREFIX  = f"[{config_name}]"

# Taille max d'un trade en mainnet — sécurité supplémentaire
...
def wait_until_next_check(timeframe: str, check_time_utc: str | None, interval_min: int | None):
    ...
    if check_time_utc:
        ...
        # Daily/weekly : une vérification par période — intraday : mi-période
...
def run():
    ...
    while True:
        try:
            ...
            size_pct  = min(cfg.get("size_pct", 10), 20)  # plafond 20% en mainnet
            ...
            # On fusionne les indicateurs entrée + sortie pour un seul calcul
            ind_merged = {
            ...
            # ── Fermeture de position ──────────────────────────────────────
                ...
                # Détermine la raison de sortie (priorité : SL > TP > signal)
            ...
            log(f"{BOT_PREFIX} Bot MAINNET arrêté (Ctrl+C)")
            break
