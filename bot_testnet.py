"""
bot_testnet.py
Trading sur Binance TESTNET — faux argent, vrais ordres.
Même logique que bot_local.py mais exécute de vrais ordres sur testnet.binance.vision.
...
Lancement : python bot_testnet.py [--config bot_state_testnet_long.json]
"""
...
# Sur Railway, start.py passe le chemin absolu : /data/bot_state_testnet_long.json
# ---------------------------------------------------------------------------
...
# Préfixe de log : "bot_state_testnet_long.json" → "[TESTNET-LONG]"
...
def next_sleep(timeframe: str, check_time_utc, interval_min) -> int:
    ...
    - interval_min : synchronise sur les multiples depuis minuit UTC
      ex: 60 min → checks à 00:00, 01:00, 02:00... + buffer
    - check_time_utc : heure UTC fixe quotidienne ex "00:01"
    - fallback : SLEEP_MAP + buffer
    """
...
# ---------------------------------------------------------------------------
def run():
    log(f"{BOT_PREFIX} 🧪 Bot TESTNET démarré", max_logs=5000)
    first_run = True
    ...
    while True:
        try:
            ...
            # Log signal — terminal uniquement
            ...
            if pos is None and entry_signal:
                ...
                if size < 10:
                    log(f"{BOT_PREFIX} ⚠️ Solde insuffisant ({balance:.2f} USDT)", max_logs=5000)
                else:
                    qty = round(size / exec_price, 6)
                    res = client.buy(symbol, qty) if not is_short else client.sell(symbol, qty)
                    if res and res["ok"]:
                        ...
                        log(f"{BOT_PREFIX} ✅ {'LONG' if not is_short else 'SHORT'} ouvert "
                            f"@ {fill:.2f} | Qty: {qty:.6f} | {size:.2f} USDT",
                            max_logs=5000)
                    else:
            ...
            # ── 6. Sortie ──────────────────────────────────────────────────
                ...
                if should_exit:
                    res = client.sell(symbol, pos["qty"]) if not is_short else client.buy(symbol, pos["qty"])
                    if res and res["ok"]:
                        fill    = res.get("fill_price") or exec_price
                        pnl_pct = (fill - entry) / entry * 100 if not is_short else (entry - fill) / entry * 100
                        if not is_short:
                            pnl_usd = round(pos["qty"] * fill - pos["size_usdt"], 2)
                        else:
                            pnl_usd = round(pos["size_usdt"] - pos["qty"] * fill, 2)
                        ...
                        log(f"{BOT_PREFIX} ❌ Vente échouée : {res}", max_logs=5000)

            # ── 7. Log état final + sauvegarder ───────────────────────────
            pos         = state.get("position")
            ...
            # Fusionner les logs avant save_state pour ne pas les perdre
            fresh                = get_state()
            state["log"]         = fresh["log"]
            ...
            # ── 8. Sleep jusqu'au prochain check ──────────────────────────
