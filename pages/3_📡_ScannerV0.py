...
all_labels  = [f"{c['symbol']} — {c['name']}" for c in COINS]
ticker_map  = {f"{c['symbol']} — {c['name']}": c["ticker"] for c in COINS}
...
with pg1:
    ...
    duree     = st.number_input("Durée (nb bougies)", min_value=5, max_value=1000, value=90)
with pg2:
...
if "scan_results" in st.session_state:
    ...
    styled = (
        ...
        .applymap(color_val, subset=["Rendement (%)", "Alpha (%)"])
    )
